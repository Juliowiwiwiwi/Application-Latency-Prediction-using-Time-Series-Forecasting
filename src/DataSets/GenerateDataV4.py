...
MAX_TRAFFIC = 800
PRECURSOR_MINUTES = 30  # <-- the key new parameter
...
df.to_csv('latency_data_v4.csv', index=False)

failure_df = pd.DataFrame(failure_windows)
failure_df.to_csv('failure_windows_v4.csv', index=False)

print("\n" + "=" * 70)
print(f"✓ Generated latency_data_v4.csv ({len(df)} samples)")
print(f"✓ Generated failure_windows_v4.csv ({len(failure_windows)} incidents)")
print("=" * 70)
