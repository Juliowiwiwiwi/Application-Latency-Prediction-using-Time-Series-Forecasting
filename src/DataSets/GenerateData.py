...
# ======================================================================
# THE FIX: Realistic Deployment Bug (Day 25)
# Instead of magically adding latency, the bad deployment thrashes 
# the CPU and locks up the database. This gives the AI the warning signs!
# ======================================================================
start_bug = 25 * 24 * 12
end_bug = start_bug + 8 * 12
cpu_usage[start_bug:end_bug] = np.clip(cpu_usage[start_bug:end_bug] + 50, 0, 99.9)
db_query_time[start_bug:end_bug] *= 3.0

cpu_usage = np.clip(cpu_usage, 5, 99.9)
memory_usage = np.clip(memory_usage, 10, 99.9)

# Calculate Latency (Latency will now naturally spike during Day 25!)
p50_latency = []
...
cpu_risk = np.maximum(0, (cpu_usage - 80) / 20)           
db_risk = np.maximum(0, (db_query_time - 35) / 30)        
latency_risk = np.maximum(0, (p99_latency - 300) / 500)   
