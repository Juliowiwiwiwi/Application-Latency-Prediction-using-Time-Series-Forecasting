"""
Production-Grade Latency Dataset Generator v3
==============================================
Bigger version of v2: 150 days instead of 90, and 4 instances of EACH
failure type instead of 2 (24 total incidents instead of 12).

Why this matters: the model in v2 only saw 2 examples of the most extreme
failure ("Cascading Failure", latency_impact~1.9-2.0) and only 3 spikes
total landed in the test set. That's not enough examples for a model to
confidently learn to reproduce the FULL HEIGHT of rare, extreme spikes —
it plays it safe and underpredicts them (regression-to-the-mean).

v3 fixes this by:
  - More days (150 vs 90) -> more raw signal, more daily/weekly cycles
  - 4x each failure type (24 total) -> more learning examples per pattern,
    and more examples specifically in the held-out test set
  - Intensity varied per instance (not identical every time) so the model
    learns a spread of magnitudes, not just one canonical shape
"""
...
# ======================================================================
# 3. INJECT FAILURE PATTERNS (4x each = 24 total)
# ======================================================================
failure_windows = []
...
print("\nInjecting failure patterns (4 instances per type):")

# --- Memory Leak (progressive, 4 instances) ---
...
# --- CPU Spike (sudden, 4 instances, varied intensity incl. one extreme) ---
...
inject_failure("CPU Spike #3", 84, 7, cpu_mult=2.40, mem_delta=12, db_mult=1.90, impact_on_latency=2.00)  # extreme
inject_failure("CPU Spike #4", 120, 6, cpu_mult=2.10, mem_delta=9, db_mult=1.70, impact_on_latency=1.70)

# --- Database Lock (4 instances) ---
...
# --- Cascading Failure (4 instances, multiple EXTREME magnitude examples) ---
...
inject_failure("Cascading Failure #3", 105, 10, cpu_mult=1.95, mem_delta=40, db_mult=2.70, impact_on_latency=2.30)  # extreme
inject_failure("Cascading Failure #4", 141, 9, cpu_mult=1.85, mem_delta=37, db_mult=2.55, impact_on_latency=2.10)  # extreme

# --- Thermal Throttle (4 instances) ---
...
df.to_csv('latency_data_v3.csv', index=False)

failure_df = pd.DataFrame(failure_windows)
failure_df.to_csv('failure_windows_v3.csv', index=False)

print("\n" + "=" * 70)
print(f"✓ Generated latency_data_v3.csv ({len(df)} samples)")
print(f"✓ Generated failure_windows_v3.csv ({len(failure_windows)} incidents)")
print("=" * 70)
...
# Quick split-coverage check so you know how many failures land in each split
n = len(df)
